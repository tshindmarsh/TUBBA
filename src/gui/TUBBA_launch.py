...
class NewProjectWindow(QWidget):
    ...
    def initUI(self):
        ...
        # Populate the dropdown from src/features
        script_dir = os.path.join(os.path.dirname(__file__), '..', 'features')
        if os.path.isdir(script_dir):
            scripts = [f for f in os.listdir(script_dir) if f.endswith('.py') and f != '__init__.py']
            ...
            for i, folder in enumerate(self.folders):
                ...
                try:
                    ...
                    if vidInfo['status'] > 0:
                        ...
                        print('✅', folder_name, 'processed successfully.')
                    else:
                        processed_list.append(f"{folder_name}  ✗")
                        print('❌', folder_name, 'failed to process.')
